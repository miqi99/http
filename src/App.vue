...
watch(
  [persistConfig, url, protocol, method, accessMode, paramType, headers, cookie, cookieEnabled, formParams, jsonBody, formatRequestJson, formatResponseJson, wsUrl, wsProtocol],
  () => {
    if (!initialized.value) return
    if (!persistConfig.value) { localStorage.removeItem(STORAGE_KEY); return }
    const config = {
      persistConfig: persistConfig.value, url: url.value, protocol: protocol.value,
      method: method.value, accessMode: accessMode.value, paramType: paramType.value,
      ...
      wsUrl: wsUrl.value, wsProtocol: wsProtocol.value,
    }
...
function removeFormParam(index) {
  if (formParams.value.length > 1) formParams.value.splice(index, 1)
}
...
function buildHeaders() {
  ...
  headers.value.forEach(item => {
    ...
    if (key) requestHeaders[key] = item.value
  })
...
function buildBody() {
  ...
  if (paramType.value === 'form') {
    ...
    formParams.value.forEach(item => {
      const key = item.key.trim()
      if (key) formData.append(key, item.value)
    })
    return formData
  }
  return undefined
...
function validateJsonBeforeSend() {
  ...
  if (!jsonBody.value.trim()) {
    response.value = 'JSON 参数不能为空'; responseTab.value = 'body'; return false
  }
          ...
          <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div class="flex items-center gap-1 border-b border-slate-100 px-4 py-2.5">
              <button
                ...
                :class="activeTab === 'http' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'"
                class="rounded-lg px-3 py-1.5 text-xs font-semibold transition"
              >HTTP</button>
              <button
                type="button"
                @click="activeTab = 'ws'"
                :class="activeTab === 'ws' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'"
                class="rounded-lg px-3 py-1.5 text-xs font-semibold transition"
              >WebSocket</button>
            </div>

            <!-- HTTP 表单 -->
            <div v-if="activeTab === 'http'" class="p-4">
        ...
        <div v-else class="overflow-hidden rounded-xl border border-slate-200">
          ...
          <div class="max-h-[600px] overflow-y-auto">
            ...
            >
              ...
              <div>
                <span class="inline-block rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{{ item.paramType }}</span>
              </div>
